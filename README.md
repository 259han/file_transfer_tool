...
- **2025-01-26**: **用户管理系统**
  - 新增完整的用户认证系统，支持用户名/密码认证
  - 实现4级权限控制系统（读取、写入、删除、管理员）
  - 添加API密钥支持，提供双重认证机制
  - 包含用户管理CLI工具，支持用户增删改查、密钥生成和认证测试
  - 密码采用SHA-256+随机盐加密存储，确保账户安全
  - 用户数据存储在`data/auth/`目录，支持权限隔离
