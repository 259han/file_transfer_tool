...
TransferResult ClientCore::upload(const TransferRequest& request) {
    ...
    while (remaining > 0) {
        ...
        // 再次验证连接状态 - 不可恢复错误
        if (!socket_ || !socket_->is_connected()) {
            result.error_message = "Socket connection lost before sending data";
            LOG_FATAL("%s", result.error_message.c_str());
            is_connected_ = false;
            std::abort();
        }
...
TransferResult ClientCore::download(const TransferRequest& request) {
    ...
    if (!local_path.parent_path().empty()) {
        try {
            ...
            result.error_message = "Failed to create directory: " + std::string(e.what());
            LOG_ERROR("%s", result.error_message.c_str());
            return result;
        }
    }
    ...
    // 发送前再次检查连接状态 - 不可恢复错误
    if (!socket_ || !socket_->is_connected()) {
        result.error_message = "Socket connection lost before sending request";
        LOG_FATAL("%s", result.error_message.c_str());
        is_connected_ = false;
        std::abort();
    }
    ...
    const int max_empty_responses = 3;
    
    while (!last_chunk) {
        // 再次检查连接状态 - 不可恢复错误
        if (!socket_ || !socket_->is_connected()) {
            result.error_message = "Socket connection lost during download";
            LOG_FATAL("%s", result.error_message.c_str());
            is_connected_ = false;
            file.close();
            std::abort();
        ...
        if (length_value > 0) {
            ...
            while (retry_count < max_retries && !received) {
                err = socket_->recv_all(payload.data(), payload.size());
                if (err == network::SocketError::SUCCESS) {
                    received = true;
                } else if (err == network::SocketError::TIMEOUT) {
                    retry_count++;
                    LOG_WARNING("Timeout receiving payload, retry %d/%d", retry_count, max_retries);
                    std::this_thread::sleep_for(std::chrono::milliseconds(200));
                } else {
            ...
            if (!received) {
                result.error_message = "Failed to receive payload after retries";
                LOG_ERROR("%s", result.error_message.c_str());
                file.close();
                return result;
            }
